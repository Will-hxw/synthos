...
# 或者，检查 Ollama 后启动全部服务，并自动创建 ngrok 公网 URL（使用生产构建静态预览）
bash run.sh
...
| `bash run.sh` | 完整开发环境 + 生产构建静态预览 + ngrok 公网转发 | orchestrator, preprocessing, ai-model, data-provider, webui-backend, static webui preview, ngrok |
