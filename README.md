...
```powershell
Copy-Item .\synthos_config.example.json .\synthos_config.json
```

`synthos_config.json` 与 `synthos_config_override.json` 是本地私有配置，已被 `.gitignore` 忽略，禁止提交真实 `dbKey`、`apiKey`、邮箱密码、ngrok token 等敏感信息。完整字段格式请参考 [`common/services/config/schemas/GlobalConfig.ts`](./common/services/config/schemas/GlobalConfig.ts)。
...
`pnpm dev:all`、`pnpm dev:backend`、`pnpm dev:webui` 和 `pnpm dev:forwarder` 启动前会只读检查 MongoDB TCP 可达性，地址读取顺序为 `SYNTHOS_MONGODB_URL` → `MONGODB_URL` → `mongodb://localhost:27017/synthos`。`pnpm dev:config` 是配置面板轻量模式，不强制检查 MongoDB。

配置面板仅面向可信本地开发环境使用；它会读取和保存本机配置文件，本轮未引入鉴权，不应暴露到公网或不可信网络。
