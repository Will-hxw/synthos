...
只考虑功能正确性、性能效率、结果准确度和交互体验问题。安全、权限、泄露、敏感信息、成本控制类问题不纳入
...
| ID | 优先级 | 状态 | 模块 | 问题摘要 |
| --- | --- | --- | --- | --- |
| BUG-001 | P1 | 已完成 | 最新话题 | `/latest-topics` 查询链路在后端全量取数后内存过滤、排序、分页，且逐条查已读/收藏，导致页面慢 |
| BUG-002 | P1 | 已完成 | 报告生成 | 日报/周报按摘要更新时间过滤 topic，导致报告时间范围不准确 |
| BUG-003 | P1 | 已完成 | 报告生成 | `pending`/`failed` 报告会永久阻止同周期自动重试 |
| BUG-004 | P1 | 已完成 | 兴趣分 | 兴趣分读取把 `0` 当缺失，且无法正确读取非 V1 分数 |
| BUG-005 | P1 | 已完成 | 报告统计 | 报告用随机 `sessionId` 猜 `groupId`，群统计不可信 |
| BUG-006 | P1 | 已完成 | 前端路由 | 去除首页、聊天记录、系统监控相关前端页面入口和路由 |
| BUG-007 | P1 | 已完成 | AI 聊天 | `/ai-chat` 三个 tab URL/按钮切换不稳定，出现切换失败或页面抖动 |
| BUG-008 | P2 | 已完成 | Agent | 取消/切换会话没有真正中止 ai-model 执行，可能写入过期回复 |
| BUG-009 | P2 | 已完成 | 最新话题 | `endDate` 只包含当天 00:00，用户语义上应包含整天 |
| BUG-010 | P2 | 已完成 | AI 流水线 | 多处 group/session/topic 串行 DB 查询和逐条写入，数据稍多会拖慢任务 |
| BUG-011 | P2 | 已完成 | 数据接入/摘要 | 图片、语音、文件消息多数只是占位符，影响总结和搜索准确度 |
| BUG-012 | P2 | 已完成 | 配置页 | `/config` 左侧导航栏太短，需要拉长，避免用户额外滚动导航栏 |
| BUG-013 | P3 | 已完成 | 前端构建 | 主 chunk 过大，首屏加载体验差 |
| BUG-014 | P3 | 已完成 | 报告页 | 报告页 URL 深链分页可能被初始化副作用重置 |
| BUG-015 | P3 | 已完成 | AI 聊天 | 侧边栏“分享/导出”按钮是空功能 |
...
- 修复提交：`dbd3b91 perf(latest-topics): paginate topic queries in database`
- 修复内容：将搜索、排序、分页、已读排除、收藏包含下推到数据库查询；已读/收藏状态改为批量读取 key 列表后参与查询过滤。
- 验证结果：全量构建通过、全量测试通过；真实接口全时间范围约 3600 条 topic 时当前页查询热请求约 186-222ms；浏览器打开 `/latest-topics?startDate=2026-01-01&endDate=2026-02-06` 可正常渲染，控制台 0 error。
...
- 修复内容：报告候选 topic 改为使用 `AgcDbAccessService.getLatestTopicRecordsByTimeRange(timeStart, timeEnd)`，按聊天消息/session 实际时间命中报告周期，不再 `selectAll()` 后按摘要 `updateTime` 内存过滤。
- 兼容边界：不变更报告 API、数据库结构和前端返回结构。
- 验证方式：新增报告生成任务单测，覆盖摘要更新时间在报告范围外但聊天命中周期仍进入报告，以及不再从全量摘要中按 `updateTime` 捞取未命中聊天时间的 topic。
...
- 修复内容：`GenerateReport` 改为按 `type + timeStart + timeEnd` 读取同周期报告；只有 `summaryStatus = success` 时跳过，`pending`/`failed` 会复用原 `reportId` 原地重试。
- 兼容边界：保留已有 `isReportExists` 行为，新增精确周期读取方法供报告生成链路使用。
- 验证方式：新增 `ReportDbAccessService` 和报告生成任务单测，覆盖成功报告阻止重复生成、`pending`/`failed` 报告允许重试并复用原 `reportId`。
...
- 修复内容：`getInterestScoreResult` 改为用 SQL 别名读取当前版本分数列，并使用 `??` 保留合法的 `0` 分。
- 验证方式：新增 `InterestScoreDbAccessService` 单测，覆盖 `0` 分、`version = 2` 和查不到结果返回 `null`。
...
- 修复内容：报告统计直接使用摘要查询联表返回的真实 `chat_messages.groupId`，删除 `sessionId.includes(groupId)` 猜测逻辑；缺失群组时明确降级为 `unknown`。
- 活跃时段：`mostActiveHour` 改为使用真实 session 的 `timeEnd` 计算，不再依赖摘要更新时间。
- 验证方式：新增报告生成任务单测，覆盖真实 `groupId` 统计、`unknown` 降级路径和基于 session 结束时间的最活跃时段计算。
...
- 修复内容：移除首页、聊天记录、系统监控的导航入口；旧 URL 重定向到 `/latest-topics`；删除对应前端页面源码和系统监控前端 API/mock/type。
- 保留边界：后端接口、数据库和数据处理逻辑未删除。
...
- 复现结果：从 `/ai-chat?tab=search` 点击 `智能Agent` 或 `AI问答` 后，URL 仍停留在 `?tab=search`，页面内容会被拉回语义搜索。
- 根因：URL 初始化 effect 依赖 `handleSelectSession`，而 `handleSelectSession` 会随 `activeTab` 变化重新生成；每次 tab 切换都会再次执行 URL 初始化，把旧 URL 参数反写回组件状态。
- 修复内容：tab 初始值直接从 URL 同步；URL 初始化只执行一次；tab 切换集中到 `handleTabChange`；URL 同步只写入当前 tab 适用的 query 参数，并在离开 Agent 时清理 `conversationId`。
- 验证结果：`/ai-chat`、`/ai-chat?tab=search`、`/ai-chat?tab=agent` 均可稳定直达；连续 20 次按钮切换后 URL、选中 tab、主内容一致，控制台 0 个应用错误。
...
- 修复内容：将 `AbortSignal` 从 webui-backend 的 SSE 连接关闭/取消链路贯穿到 tRPC `agentAskStream`、`RagRPCImpl.agentAsk` 和 `LangGraphAgentExecutor.executeStream`，取消后停止继续发出事件，并在 assistant 消息落库前再次检查取消状态。
- 前端处理：Agent 聊天面板增加取消按钮；切换会话时会中止旧请求，并通过请求序号忽略旧流事件，避免旧 token 或 done 事件回写当前会话。
- 兼容边界：不改变 Agent SSE 事件格式、REST API 入参和历史消息返回结构。
- 验证方式：新增 `RagRPCImpl` 单测覆盖取消后不写入 assistant 消息；前端类型检查和生产构建通过。
...
- 修复内容：`/latest-topics` 将用户选中的结束日期转换为次日 00:00 作为请求结束边界，默认结束日期仍展示为今天。
- 影响范围：只调整前端日期到 UNIX 毫秒时间戳的转换语义，接口字段和返回结构不变。
...
- 修复内容：增强 QQ 消息解析文本化逻辑，图片无 `imageText` 时输出图片类型、尺寸、MD5 或“暂无文字描述”等元信息；语音优先输出 `pttText`；文件输出文件名和大小；视频、卡片、XML、动态、通话、系统消息和新版表情尽量输出可读文本。
- 兼容边界：不新增 OCR、语音转写、文件摘要等外部依赖；仍只写入 `messageContent`，下游预处理、摘要和搜索链路保持现有字段契约。
- 验证方式：更新 `QQProvider.unit.test.ts`，覆盖图片识别文本、语音转写、文件消息和卡片消息可读文本提取。
...
- 修复内容：配置页侧边栏改为基于视口高度的 sticky 布局，内部导航区域填满剩余高度。
- 影响范围：只调整 `/config` 左侧导航布局，不改变配置表单逻辑。
...
- 修复内容：前端路由页改为 `React.lazy` 按需加载，并提供稳定 loading fallback；Vite `manualChunks` 拆分 React、Markdown、动画、HeroUI、ECharts 与 zrender；群组页从全量 `echarts` import 改为按需注册折线图、网格、图例、提示框和 Canvas 渲染器。
- 验证结果：`pnpm --filter vite-template build` 通过，构建不再出现 chunk 超限警告；入口 JS chunk 约 8.52 kB，最大图表 chunk 约 334.65 kB。
- 兼容边界：不改变路由路径、页面业务逻辑和 API 调用契约。
...
- 修复内容：移除 `selectedType` 初始化后无条件重置页码的 effect，只在用户点击报告类型 Tab 时重置到第 1 页。
- 验收目标：`/reports?type=weekly&page=3` 初始化时保留第 3 页，手动切换类型仍回到第 1 页。
...
- 修复内容：隐藏 AI Chat 历史会话菜单中的“分享”和“导出”空入口，并移除对应 TODO handler 与 props 透传。
- 验收目标：会话更多菜单不再出现无结果按钮，控制台不再输出占位日志。
...
1. `BUG-008`：修正 Agent 取消/切换会话后仍继续执行的问题。
2. `BUG-010`：批量化 AI pipeline 中的串行查询和逐条写入。
3. `BUG-011`：提升媒体消息进入摘要和搜索的准确度。
4. `BUG-013`：优化前端主 chunk 体积和首屏加载体验。
